...
for (const project of projects) {
  ...
  if (!pkg.scripts || !pkg.scripts.test) {
    console.log(`[skip] ${project.name}: no test script`);
    continue;
  }

  console.log(`[test] ${project.name}`);
  const result = spawnSync(npmCommand, ['run', '--workspace', project.name, 'test'], {
    cwd: root,
    env: npmEnv,
    stdio: 'inherit',
...
console.log('\nTests completed for all available projects.');
